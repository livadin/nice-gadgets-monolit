...
export const CheckoutPage: React.FC = () => {
  ...
  // 1. Отримуємо дані та функції зі стору
  ...
  // Стилі для карток (ліва і права колонки)
  ...
  return (
    <section className="py-0 min-h-screen lg:py-8 relative">
      ...
      <div className="relative z-10 px-4 pb-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8 w-full max-w-[1200px] mx-auto">
          ...
          <article className={cardClassName}>
            ...
            {/* Pay Button */}
            <PrimaryButton
                buttonText={cn(`Pay Now ${total.toFixed(2)}`)}
                onClick={handlePay}
                className={cn(
                "w-full mt-6 bg-[#905BFF] hover:shadow-white/50 text-white font-bold py-4 rounded-xl text-xl shadow-lg transition-transform active:scale-[0.98] cursor-pointer",
                { "opacity-50 cursor-not-allowed grayscale": cart.length === 0 }
              )}
              />
          </article>
