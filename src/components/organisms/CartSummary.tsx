...
export const CartSummary: React.FC<CartProductsProps> = ({ cartProducts }) => {
  ...
  const total = cartProducts.reduce((sum, product) => {
    if (product.priceDiscount) {
      return sum + product.priceDiscount;
    }
    return sum + product.priceRegular;
  }, 0);
  ...
  return (
    <div className="flex flex-col items-center justify-center box-border border border-element p-6 h-[190px] lg:h-[206px] lg:w-[368px]">
      <h3 className="text-[32px] text-primary font-extrabold">
        {`$${total}`}
      </h3>
      <div className="
      ...
      "
      >
        <span className="text-sm font-semibold text-secondary">{`Total for ${cartProducts.length} items`}</span>
      </div>
      <PrimaryButton
